				...
				<div class="w-full max-w-lg">
					...
					<form @submit.prevent="login" class="max-w-sm m-4 p-10 bg-gray-100 rounded shadow-xl">
							...
							<div class="mt-2">
								...
								<input class="w-full px-5 py-1 text-gray-700  border border-black bg-gray-300 rounded focus:outline-none focus:bg-white" v-model="password" type="password" id="password" placeholder="Write Password" arial-label="password" required>
							</div>
							<div v-if="errors" class="py-2"> <span class="text-red-500 font-semibold">
                    {{ errors }}
                </span>
						 </div>
							<div class="mt-4 items-center flex justify-between">
								<button class="px-4 py-1 text-white font-light tracking-wider bg-gray-600 hover:bg-gray-800 rounded" type="submit">Login</button>
